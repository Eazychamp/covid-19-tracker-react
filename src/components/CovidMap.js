...
const CovidChart = props => {
    ...
    const { covidData=[], onHovered = null, onHoveredOut = null,tooltipContent } = props
    ...
    >
        <Geographies geography={INDIA_TOPO_JSON}>
          {({ geographies }) =>
            geographies.map(geo => {
              const current = data &&  data.find(s => s.id === geo.id);
              return (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
                //   fill={'blue'}
                  ...
                  onMouseEnter={() => onHovered(geo, current)}
